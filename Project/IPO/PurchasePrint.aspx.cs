...
public partial class QuotePrint : System.Web.UI.Page
{
    ...
    private void LoadIPOInfo(string code)
    {
        ...
        if (dt.Rows.Count > 0 && code.Length>0)
        {
            ...
            lblPurchaseDate.Text = Convert.ToDateTime(row["PurchaseDate"]).ToString("yyyy-MM-dd");
            lblOnNetUpLimit.Text = ((row["UpLimitVolOnNet"] == DBNull.Value) ? "" : Convert.ToDouble(row["UpLimitVolOnNet"]).ToString());
            lblUnderNetDownLimit.Text = ((row["DownLimitVolUnderNet"] == DBNull.Value) ? "" : Convert.ToDouble(row["DownLimitVolUnderNet"]).ToString());
            lblUnderNetUpLimit.Text = ((row["UpLimitVolUnderNet"] == DBNull.Value) ? "" : Convert.ToDouble(row["UpLimitVolUnderNet"]).ToString()); 
            ...
            tbPrice.Text = ((row["IssuePrice"] == DBNull.Value) ? "" : Convert.ToDouble(row["IssuePrice"]).ToString());

            double totalCapitalBeforeIPO = 0, issueCapitalWithTransferred = 0, transferredCapital = 0;
            if (row["TotalCapitalBeforeIPO"] != DBNull.Value)
                totalCapitalBeforeIPO = Convert.ToDouble(row["TotalCapitalBeforeIPO"]);
            if (row["ActualIssuedCapital"] != DBNull.Value)
                issueCapitalWithTransferred = Convert.ToDouble(row["ActualIssuedCapital"]);
            if (row["ActualTransferredCapital"] != DBNull.Value)
                transferredCapital = Convert.ToDouble(row["ActualTransferredCapital"]);

            lblTotalCapital.Text = "发行前总股本" + totalCapitalBeforeIPO.ToString("N0") + "万股，发行后总股本" + (totalCapitalBeforeIPO + issueCapitalWithTransferred - transferredCapital).ToString("N0") + "万股";
        }
        else
        {
            lblCode2.Text = "";
            ...
            lblOnNetUpLimit.Text = "";
            lblUnderNetDownLimit.Text = "";
            lblUnderNetUpLimit.Text = "";
            //lblQuoteStart.Text = "";
            //lblQuoteEnd.Text = "";
