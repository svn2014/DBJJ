...
public partial class PurchaseOnNet : System.Web.UI.Page
{
    ...
    private string getTableHtml(string fundcode, DataTable dtIPOs, DataTable dtIPOUndernet)
    {
        ...
        string table = @"
                ...
                <tr>
                    <td colspan='8'><strong style='font-size: large; font-weight: bold'>" + fundName + @"</strong></td>
                </tr>
                <tr>
                    <td colspan='2'>净资产</td>
        ...
        if (dtIPOs.Rows.Count > 0)
        {
            ...
            foreach (DataRow row in dtIPOs.Rows)
            {
                ...
                double maxVolume = ((row["UpLimitVolOnNet"] == DBNull.Value) ? 0 : Convert.ToDouble(row["UpLimitVolOnNet"])) * 10000;
                ...
                if (dtTmp.Rows.Count > 0)
                {
                    ...
                    tr += @"
                        <tr>
                            <td><input id='chkJoin" + unicode + @"' type='checkbox' disabled " + radiostyle + @" >禁止参与</input></td>
                            <td>" + trName + @"</td>
                            <td align='right'>" + trPrice.ToString("N2") + @"<input type='hidden' value='" + trPrice + @"' id='calPrice" + unicode + @"' /></td>
                            <td align='right'>
                                <input id='calVolume" + unicode + @"' type='text' value='0' style='width:80px;text-align:right; background-color: #FFFF00; font-size: large; font-weight: bold;' disabled/>
                            </td>
                            <td align='right'><span id='calAmount" + unicode + @"'></span></td>
                            <td align='right'><span id='calPct" + unicode + @"'></span></td>
                            <td align='center'><input type='radio' name='group1_" + unicode + @"' " + radiostyle + @">是</input><input type='radio' name='group1_" + unicode + @"'  " + radiostyle + @">否</input></td>
                ...
                { 
                    ...
                    tr += @"
                        <tr>
                            ...
                            <td align='right'>" + trPrice.ToString("N2") + @"<input type='hidden' value='" + trPrice + @"' id='calPrice" + unicode + @"' /></td>
                            <td align='right'>
                                <input id='calVolume" + unicode + @"' type='text' value='" + maxVolume + @"' style='width:80px;text-align:right; background-color: #FFFF00; font-size: large; font-weight: bold;' />
                            </td>
                            <td align='right'><span id='calAmount" + unicode + @"'></span></td>
                            <td align='right'><span id='calPct" + unicode + @"'></span></td>
                            <td align='center'><input type='radio' name='group1_" + unicode + @"' " + radiostyle + @">是</input><input type='radio' name='group1_" + unicode + @"'  " + radiostyle + @">否</input></td>
