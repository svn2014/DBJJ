...
namespace ModelLib
{
    ...
    /// 基金仓位测算模型
    /// </summary>
    public class ModelFundPositionEstimate : AModel
    {
        #region 枚举
        enum IndexType
        { 
        ...
        public override void SetParameters(Hashtable ht)
        {
            try
            {
        ...
        public override DataSet GetResult()
        {
            try
            {
                ...
                szzs.LoadData(DataInfoType.TradingPrice);
                ...
                for (int i = 0; i < dtEquity.Rows.Count; i++)
                {
                    ...
                    for (int k = 0; k < 2; k++)
                    {
                        ...
                        if (r["Value1"] != DBNull.Value && r["Value2"] != DBNull.Value)
                        {
                            ...
                            ATimeItem idxPx = szzs.TradingPrice.AdjustedTimeSeries.Find(delegate(ATimeItem s) { return s.TradeDate == tradedate; });
                            if (idxPx != null)
                            ...
                            int idx = mf.FundReport.AdjustedTimeSeries.FindLastIndex(delegate(ATimeItem s) { return s.TradeDate >= tradedate.AddDays(20); });
                            ATimeItem mfrpt = null;
                            if (idx >= 0)
                                mfrpt = mf.FundReport.AdjustedTimeSeries[idx];
                            else
                                mfrpt = mf.FundReport.AdjustedTimeSeries[0];
        ...
        #region 方法
        public ModelFundPositionEstimate()
        {
            this.Name = "基金仓位测算模型";
        ...
        private MWNumericArray GetFundTimeSeries(int yieldspan, FundAssetCategory category)
        {
            ...
            if (WholeFund == null)
            {
                MutualFundSelector mfs = new MutualFundSelector();
                MutualFundGroup mfg= null;
                switch (category)
                {
                    case FundAssetCategory.Equity:
                        mfg = mfs.GetActiveEquityOpenFund();    //股票型
                        break;
                    case FundAssetCategory.Hybrid:
                        mfg = mfs.GetActiveHybridOpenFund();    //混合型
                        break;
                    default:
                        mfg = mfs.GetActiveEquityOpenFund();    //股票型
                        break;
                ...
                mfg.LoadData(DataInfoType.FundNetAssetValue);
                mfg.LoadData(DataInfoType.SecurityReport);
            ...
            int rows = WholeFund.TradingNAV.AdjustedTimeSeries.Count;
            ...
            for (int i = 0; i < rows; i++)
            {
                ...
                {
                    case 1:
                        d = WholeFund.TradingNAV.AdjustedTimeSeries[i].UpAndDown.KLineDay1;
                        break;
                    case 2:
                        d = WholeFund.TradingNAV.AdjustedTimeSeries[i].UpAndDown.KLineDay2;
                        break;
                    case 3:
                        d = WholeFund.TradingNAV.AdjustedTimeSeries[i].UpAndDown.KLineDay3;
                        break;
                    case 4:
                        d = WholeFund.TradingNAV.AdjustedTimeSeries[i].UpAndDown.KLineDay4;
                        break;
                    case 5:
                        d = WholeFund.TradingNAV.AdjustedTimeSeries[i].UpAndDown.KLineDay5;
                        break;
                    default:
                        d = WholeFund.TradingNAV.AdjustedTimeSeries[i].UpAndDown.KLineDay1;
                        break;
                ...
                TradeDayList.Add(WholeFund.TradingNAV.AdjustedTimeSeries[rows - i - 1].TradeDate);
            }
        ...
        {
            if (this.OutputSeriesList.Count > 0)
            {
                foreach (OutputSeries os in this.OutputSeriesList)
                {
                    os.DebugPrint();
